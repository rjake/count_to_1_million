...
# syllables per second
    sps <- ceiling(sum(combine_text$count[1:100])/25)
    
#totals
    sum(combine_text$count[1:100])/sps # 100 = 274 syllables in 25 seconds, 11 syllables a second
    sum(combine_text$count[1:1e3])/sps/60 # 1K = 5513 syllables in 8.35 min
    sum(combine_text$count[1:1e4])/sps/60/60 # 10K = 83103 syllables in 2.09 hours
    sum(combine_text$count[1:1e5])/sps/60/60 # 100K = 1,020,005 syllables in 26 hours
    sum(combine_text$count[1:1e6])/sps/60/60/24 # 1M = 13,018,000 syllables in 329 hours or 13.7 days

# 1 min rest for every 5 min of speaking, max 8 hours a day
    (sum(combine_text$count[1:1e6])/11/60 # total min (19,724)
        * (6/5) # 6 min for every 5 (23,669 min)
        / 60    # convert to hours (294 hours)
        / 8     # max 8 hours per day (49.3 days)
    )
```
